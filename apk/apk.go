...
func (k *Apk) Icon(resConfig *apkparser.ResTableConfig) (image.Image, error) {
	iconPath, err := k.manifest.App.Icon.WithResTableConfig(resConfig).String()
	if err != nil {
		return nil, err
	}
	if apkparser.IsResID(iconPath) {
...
func (k *Apk) Label(resConfig *apkparser.ResTableConfig) (s string, err error) {
	s, err = k.manifest.App.Label.WithResTableConfig(resConfig).String()
	if err != nil {
		return
	}
	if apkparser.IsResID(s) {
...
func (k *Apk) parseManifest() error {
	...
	xmlfile, err := apkparser.NewXMLFile(bytes.NewReader(xmlData))
	if err != nil {
...
func (k *Apk) parseResources() (err error) {
	...
	k.table, err = apkparser.NewTableFile(bytes.NewReader(resData))
	return
}
